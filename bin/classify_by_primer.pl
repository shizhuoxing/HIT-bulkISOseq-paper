...
die &usage() if ((!defined $blastm7) or (!defined $ccsfa) or (!defined $umilen) or (!defined $min_primerlen) or (!defined $min_isolen) or (!defined $outdir) or (!defined $sample) or (defined $help));
...
open IN,"$ccsfa";
open FL,">$outdir/$sample.isoseq_flnc.fasta";
open NFL,">$outdir/$sample.isoseq_nfl.fasta";
open XLS,">$outdir/$sample.isoseq_flnc.polyAtail.xls";
